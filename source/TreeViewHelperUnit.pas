...
uses
  ...
  procedure LoadTreeView(tv: TTreeView; var AList: TRangeElementRecList; out count: Integer);
...
begin
  ...
  RangeNode := nil;
  ltmpList := TRangeElementRecList.Create;
  try
    ...
    begin
      ...
      if OldElementId <> AList.Items[i].element_id then
        FillChildNodes(RangeNode, AList.Items[i].element_id, ltmpList);
      OldElementId := AList.Items[i].element_id;
    end;
  finally
    count := ltmpList.Count;
    FreeAndNil(ltmpList);
  end;
end;
